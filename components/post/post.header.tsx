...
export const PostHeader = ({ title, lead, date: dateRaw, dateUpdated }: PostHeaderProps) => {
  ...
  return (
    <header className="mb-16 grid">
      ...
      <p className="!mt-0 !mb-8 !text-2xl !leading-relaxed tracking-normal text-gray-500">{lead}</p>
      {date && (
        <time
          dateTime={new Date(date).toLocaleDateString('en-CA')}
          className="!m-0 inline-flex items-center gap-2 text-base font-medium text-gray-500"
        >
